          ...
          <div className="bg-[#FF5C00]/5 rounded-[3rem] p-12 flex flex-col md:flex-row items-center justify-between gap-12">
            <div className="max-w-xl">
              <h1 className="text-5xl md:text-7xl font-extrabold leading-tight mb-6">
                Fastest Food <span className="text-[#FF5C00]">Delivery</span> in Your City
              </h1>
              <p className="text-gray-600 text-lg mb-8">
                Hungry? We’ve got you covered. Explore the best menu items from top categories and get them delivered in minutes.
              </p>
        ...
        <section>
          {loading ? (
            ...
            selectedCategory || search ? (
              ...
              <div className="space-y-16">
                {categories.map(category => {
                  const categoryItems = menuItems.filter(item => item.category?.id === category.id);
            ...
            <div className="text-center py-24 bg-gray-50 rounded-[3rem]">
              ...
              <button onClick={() => {setSearch(''); setSelectedCategory(null)}} className="mt-4 text-[#FF5C00] font-bold">Clear Filters</button>
            </div>
          )}
        </section>
      </main>
